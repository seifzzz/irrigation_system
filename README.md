#Before Running server , change application properties, to connect your local database using mysql (url , usernaem and password)

# Plot Controller : it's used to create,configure,edit and show list of plot.
...
# Irrigation System Controller : it's used to start,stop,know system's status and irrigate just one land.
...
#Irrigation System Service : used to check if the system is working or not.

#Sensor Service : used to check ,if the sensor is available to irrigate land or not , if it's not available ,it will retry 3 times in every 30 seconed.

#Alerting System Service : is used when the system can't irrigate , just to alert the client ,if he can't start it now and inform him it will start after the 
system is available
